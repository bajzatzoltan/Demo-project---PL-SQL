-----------------------------------------------------------------------
-- interface_pack.insert_proc PROCEDURE UNIT TESZT:
-----------------------------------------------------------------------

declare
    TEST_ID varchar2(30) := '0008.001.001';
    TEST_METHOD varchar2 (60) := 'interface_pack.insert_proc';
    TEST_NAME varchar2 (120) := 'Valid par_viewsynonymname param test with current user (PROPALLADMIN) session';
    TEST_RESULT varchar2 (5);
    par_viewsynonymname varchar2(30) := 'Teszttábla 03';
    par_listoftablecolumnames varchar2 (3) := null;
    par_listofvalidatedvalues varchar2 (3) := null;
    
    expected_exception varchar2(9) := 'ORA-20000';
    exceptionmessage varchar2 (1000);
    begin
        TEST_RESULT := 'FALSE';
        execute immediate 'create role testrole';
        execute immediate 'GRANT INSERT ON PROPALLADMIN.test_table03 TO testrole';
        execute immediate 'grant testrole to PROPALLADMIN';
        interface_pack.insert_proc(par_viewsynonymname, par_listoftablecolumnames, par_listofvalidatedvalues);
        
        dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
        execute immediate 'drop role testrole';
        exception 
            when others then
                if expected_exception = substr(sqlerrm, 1, 9) then
                    TEST_RESULT := 'TRUE';
                    exceptionmessage := sqlerrm;
                else
                    TEST_RESULT := 'FALSE';
                    exceptionmessage := sqlerrm;
                end if;
                dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'drop role testrole';
end;
/

-----------------------------------------------------------------------
declare
    TEST_ID varchar2(30) := '0008.001.002';
    TEST_METHOD varchar2 (60) := 'interface_pack.insert_proc';
    TEST_NAME varchar2 (120) := 'Invalid par_viewsynonymname param test with current user (PROPALLADMIN) session';
    TEST_RESULT varchar2 (5);
    par_viewsynonymname varchar2(30) := 'Teszttábla03';
    par_listoftablecolumnames varchar2 (3) := null;
    par_listofvalidatedvalues varchar2 (3) := null;
    
    expected_exception varchar2(9) := 'ORA-20002';
    exceptionmessage varchar2 (1000);
    begin
        TEST_RESULT := 'FALSE';
        execute immediate 'create role testrole';
        execute immediate 'GRANT INSERT ON PROPALLADMIN.test_table03 TO testrole';
        execute immediate 'grant testrole to PROPALLADMIN';
        interface_pack.insert_proc(par_viewsynonymname, par_listoftablecolumnames, par_listofvalidatedvalues);
        
        dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'drop role testrole';

        exception 
            when others then
                if expected_exception = substr(sqlerrm, 1, 9) then
                    TEST_RESULT := 'TRUE';
                    exceptionmessage := sqlerrm;
                else
                    TEST_RESULT := 'FALSE';
                    exceptionmessage := sqlerrm;
                end if;
                dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'drop role testrole';

end;
/

-----------------------------------------------------------------------
-- interface_pack.update_proc PROCEDURE UNIT TESZT:
-----------------------------------------------------------------------

declare
    TEST_ID varchar2(30) := '0008.002.001';
    TEST_METHOD varchar2 (60) := 'interface_pack.update_proc';
    TEST_NAME varchar2 (120) := 'Valid par_viewsynonymname param test with current user (PROPALLADMIN) session';
    TEST_RESULT varchar2 (5);
    par_viewsynonymname varchar2(30) := 'Teszttábla 03';
    par_listoftablecolumnames varchar2 (3) := null;
    par_listofvalidatedvalues varchar2 (3) := null;
    
    expected_exception varchar2(9) := 'ORA-20000';
    exceptionmessage varchar2 (1000);
    begin
        TEST_RESULT := 'FALSE';
        execute immediate 'create role testrole';
        execute immediate 'GRANT UPDATE ON PROPALLADMIN.test_table03 TO testrole';
        execute immediate 'grant testrole to PROPALLADMIN';
        interface_pack.update_proc(par_viewsynonymname, par_listoftablecolumnames, par_listofvalidatedvalues);
        
        dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'drop role testrole';

        exception 
            when others then
                if expected_exception = substr(sqlerrm, 1, 9) then
                    TEST_RESULT := 'TRUE';
                    exceptionmessage := sqlerrm;
                else
                    TEST_RESULT := 'FALSE';
                    exceptionmessage := sqlerrm;
                end if;
                dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'drop role testrole';

end;
/
-----------------------------------------------------------------------

declare
    TEST_ID varchar2(30) := '0008.002.002';
    TEST_METHOD varchar2 (60) := 'interface_pack.update_proc';
    TEST_NAME varchar2 (120) := 'Invalid par_viewsynonymname param test with current user (PROPALLADMIN) session';
    TEST_RESULT varchar2 (5);
    par_viewsynonymname varchar2(30) := 'Teszttábla03';
    par_listoftablecolumnames varchar2 (3) := null;
    par_listofvalidatedvalues varchar2 (3) := null;
    
    expected_exception varchar2(9) := 'ORA-20002';
    exceptionmessage varchar2 (1000);
    begin
        TEST_RESULT := 'FALSE';
        execute immediate 'create role testrole';
        execute immediate 'GRANT  UPDATE ON PROPALLADMIN.test_table03 TO testrole';
        execute immediate 'grant testrole to PROPALLADMIN';
        interface_pack.update_proc(par_viewsynonymname, par_listoftablecolumnames, par_listofvalidatedvalues);
        
        dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'drop role testrole';

        exception 
            when others then
                if expected_exception = substr(sqlerrm, 1, 9) then
                    TEST_RESULT := 'TRUE';
                    exceptionmessage := sqlerrm;
                else
                    TEST_RESULT := 'FALSE';
                    exceptionmessage := sqlerrm;
                end if;
                dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'drop role testrole';

end;
/
-----------------------------------------------------------------------
-- interface_pack.delete_proc PROCEDURE UNIT TESZT:
-----------------------------------------------------------------------

declare
    TEST_ID varchar2(30) := '0008.003.001';
    TEST_METHOD varchar2 (60) := 'interface_pack.delete_proc';
    TEST_NAME varchar2 (120) := 'Valid par_viewsynonymname param test with current user (PROPALLADMIN) session';
    TEST_RESULT varchar2 (5);
    par_viewsynonymname varchar2(30) := 'Teszttábla 03';
    par_listoftablecolumnames varchar2 (3) := null;
    par_listofvalidatedvalues varchar2 (3) := null;
    
    expected_exception varchar2(9) := 'ORA-20000';
    exceptionmessage varchar2 (1000);
    begin
        TEST_RESULT := 'FALSE';
        execute immediate 'create role testrole';
        execute immediate 'GRANT DELETE ON PROPALLADMIN.test_table03 TO testrole';
        execute immediate 'grant testrole to PROPALLADMIN';
        interface_pack.delete_proc(par_viewsynonymname, par_listoftablecolumnames, par_listofvalidatedvalues);
        
        dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'drop role testrole';
        exception 
            when others then
                if expected_exception = substr(sqlerrm, 1, 9) then
                    TEST_RESULT := 'TRUE';
                    exceptionmessage := sqlerrm;
                else
                    TEST_RESULT := 'FALSE';
                    exceptionmessage := sqlerrm;
                end if;
                dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'drop role testrole';
end;
/

-----------------------------------------------------------------------


declare
    TEST_ID varchar2(30) := '0008.003.002';
    TEST_METHOD varchar2 (60) := 'interface_pack.delete_proc';
    TEST_NAME varchar2 (120) := 'Invalid par_viewsynonymname param test with current user (PROPALLADMIN) session';
    TEST_RESULT varchar2 (5);
    par_viewsynonymname varchar2(30) := 'Teszttábla03';
    par_listoftablecolumnames varchar2 (3) := null;
    par_listofvalidatedvalues varchar2 (3) := null;
    
    expected_exception varchar2(9) := 'ORA-20002';
    exceptionmessage varchar2 (1000);
    begin
        TEST_RESULT := 'FALSE';
        execute immediate 'create role testrole';
        execute immediate 'GRANT DELETE ON PROPALLADMIN.test_table03 TO testrole';
        execute immediate 'grant testrole to PROPALLADMIN';
        interface_pack.delete_proc(par_viewsynonymname, par_listoftablecolumnames, par_listofvalidatedvalues);
        
        dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'drop role testrole';
        exception 
            when others then
                if expected_exception = substr(sqlerrm, 1, 9) then
                    TEST_RESULT := 'TRUE';
                    exceptionmessage := sqlerrm;
                else
                    TEST_RESULT := 'FALSE';
                    exceptionmessage := sqlerrm;
                end if;
                dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'drop role testrole';
end;
/

-----------------------------------------------------------------------
-- interface_pack.generalselect_func FUNCTION UNIT TESZT:
-----------------------------------------------------------------------

declare
    TEST_ID varchar2(30) := '0008.004.001';
    TEST_METHOD varchar2 (60) := 'select_pack.generalselect_func';
    TEST_NAME varchar2 (120) := 'Valid par_viewsynonymname param test with current user (PROPALLADMIN) session';
    TEST_RESULT varchar2 (6);
    par_viewsynonymname varchar2(30) := 'Teszttábla 03';
    expected_sysrefcursor sys_refcursor;
    result_sysrefcursor sys_refcursor;
    expected_exception varchar2(9) := null;
    exceptionmessage varchar2 (1000);
    
    rec_exp_sysrefcur test_table03%rowtype;
    rec_res_sysrefcur "Teszttábla 03"%rowtype;
    begin
        TEST_RESULT := 'TRUE';
        execute immediate 'create role testrole';
        execute immediate 'GRANT SELECT, INSERT, UPDATE, DELETE ON PROPALLADMIN.test_table03 TO testrole';
        execute immediate 'grant testrole to PROPALLADMIN';
        insert into cashtable (cashtype_fk_id, synonymname) values (1, par_viewsynonymname);
        insert into test_table03 (testvarchar, testnumber, testnumberfraction, testdate) 
            values ('szöveg01', 100, 10.10, TO_DATE('2001/01/01 01:22:33', 'yyyy/mm/dd hh24:mi:ss')); 
        insert into test_table03 (testvarchar, testnumber, testnumberfraction, testdate) 
            values ('szöveg02', 200, 20.20, TO_DATE('2002/02/02 02:22:22', 'yyyy/mm/dd hh24:mi:ss')); 
        insert into test_table03 (testvarchar, testnumber, testnumberfraction, testdate) 
            values ('szöveg03', 300, 30.30, TO_DATE('2003/03/03 03:33:33', 'yyyy/mm/dd hh24:mi:ss')); 
        insert into test_table03 (testvarchar, testnumber, testnumberfraction, testdate) 
            values ('szöveg04', 400, 40.40, TO_DATE('2004/04/04 04:44:44', 'yyyy/mm/dd hh24:mi:ss'));
        open expected_sysrefcursor for select * from test_table03;
        result_sysrefcursor := interface_pack.generalselect_func(par_viewsynonymname);

        loop 
            FETCH expected_sysrefcursor INTO rec_exp_sysrefcur;
            FETCH result_sysrefcursor INTO rec_res_sysrefcur;
            /*DBMS_OUTPUT.put_line('result: '||
                                    rec_res_sysrefcur.id||' - '||
                                    rec_res_sysrefcur."TESZTSZÖVEG01"||' - '||
                                    rec_res_sysrefcur."TESZTSZÁM"||' - '||
                                    rec_res_sysrefcur."TESZTTÖRT"||' - '||
                                    rec_res_sysrefcur."TESZTDÁTUM"||' - ');
            DBMS_OUTPUT.put_line('expected: '||
                                    rec_exp_sysrefcur.id||' - '||
                                    rec_exp_sysrefcur.testvarchar||' - '||
                                    rec_exp_sysrefcur.testnumber||' - '||
                                    rec_exp_sysrefcur.testnumberfraction||' - '||
                                    rec_exp_sysrefcur.testdate||' - ');*/
            if rec_exp_sysrefcur.id <> rec_res_sysrefcur.id or rec_res_sysrefcur.id is null then
                TEST_RESULT := 'FALSE';
                exit;
            end if;
            if rec_exp_sysrefcur.testvarchar <> rec_res_sysrefcur."TESZTSZÖVEG01" or rec_res_sysrefcur."TESZTSZÖVEG01" is null then
                TEST_RESULT := 'FALSE';
                exit;
            end if;
            if rec_exp_sysrefcur.testnumber <> rec_res_sysrefcur."TESZTSZÁM" or rec_res_sysrefcur."TESZTSZÁM" is null then
                TEST_RESULT := 'FALSE';
                exit;
            end if;if rec_exp_sysrefcur.testnumberfraction <> rec_res_sysrefcur."TESZTTÖRT" or rec_res_sysrefcur."TESZTTÖRT" is null then
                TEST_RESULT := 'FALSE';
                exit;
            end if;if rec_exp_sysrefcur.testdate <> rec_res_sysrefcur."TESZTDÁTUM" or rec_res_sysrefcur."TESZTDÁTUM" is null then
                TEST_RESULT := 'FALSE';
                exit;
            end if;
            exit when expected_sysrefcursor%NOTFOUND;
        end loop;
        
        
        execute immediate 'truncate table TEST_TABLE03';
        execute immediate 'drop sequence '||constantsettergetter_pack.const_schemaowner||'.test_table03_id_seq';
        execute immediate 'CREATE SEQUENCE test_table03_id_seq START WITH 1 NOCACHE ORDER';
        execute immediate 'drop role testrole';
        delete from cashtable where synonymname = par_viewsynonymname;
        dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
        
        exception 
            when others then
                TEST_RESULT := 'FALSE';
                exceptionmessage := sqlerrm;
                dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'truncate table TEST_TABLE03';
                execute immediate 'drop sequence '||constantsettergetter_pack.const_schemaowner||'.test_table03_id_seq';
                execute immediate 'CREATE SEQUENCE test_table03_id_seq START WITH 1 NOCACHE ORDER';
                execute immediate 'drop role testrole';
                delete from cashtable where synonymname = par_viewsynonymname;
end;
/
-----------------------------------------------------------------------

declare
    TEST_ID varchar2(30) := '0008.004.002';
    TEST_METHOD varchar2 (60) := 'interface_pack.generalselect_func';
    TEST_NAME varchar2 (120) := 'Invalid par_viewsynonymname param test with current user (PROPALLADMIN) session';
    TEST_RESULT varchar2 (5);
    par_viewsynonymname varchar2(30) := 'Teszttábla03';
    result_sysrefcursor select_pack.select_cur;
    
    expected_exception varchar2(9) := 'ORA-20002';
    exceptionmessage varchar2 (1000);

    begin
        TEST_RESULT := 'FALSE';
        execute immediate 'create role testrole';
        execute immediate 'GRANT SELECT, INSERT, UPDATE, DELETE ON PROPALLADMIN.test_table03 TO testrole';
        execute immediate 'grant testrole to PROPALLADMIN';
        result_sysrefcursor := interface_pack.generalselect_func(par_viewsynonymname);
        
        dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'drop role testrole';
        exception 
            when others then
                if expected_exception = substr(sqlerrm, 1, 9) then
                    TEST_RESULT := 'TRUE';
                    exceptionmessage := sqlerrm;
                else
                    TEST_RESULT := 'FALSE';
                    exceptionmessage := sqlerrm;
                end if;
                dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'drop role testrole';
end;
/


-----------------------------------------------------------------------
-- interface_pack.selectall_func FUNCTION UNIT TESZT:
-----------------------------------------------------------------------
declare
    TEST_ID varchar2(30) := '0008.005.001';
    TEST_METHOD varchar2 (60) := 'select_pack.selectall_func';
    TEST_NAME varchar2 (120) := 'Valid param test  with current user (PROPALLADMIN) session';
    TEST_RESULT varchar2 (6);
    par_viewsynonymname varchar2(30) := 'Teszttábla 03';
    expected_sysrefcursor sys_refcursor;
    result_sysrefcursor sys_refcursor;
    expected_exception varchar2(9) := null;
    exceptionmessage varchar2 (1000);
    
    rec_exp_sysrefcur test_table03%rowtype;
    rec_res_sysrefcur "Teszttábla 03"%rowtype;
    begin
        TEST_RESULT := 'TRUE';
        execute immediate 'create role testrole';
        execute immediate 'GRANT SELECT, INSERT, UPDATE, DELETE ON PROPALLADMIN.test_table03 TO testrole';
        execute immediate 'grant testrole to PROPALLADMIN';
        insert into test_table03 (testvarchar, testnumber, testnumberfraction, testdate) 
            values ('szöveg01', 100, 10.10, TO_DATE('2001/01/01 01:22:33', 'yyyy/mm/dd hh24:mi:ss')); 
        insert into test_table03 (testvarchar, testnumber, testnumberfraction, testdate) 
            values ('szöveg02', 200, 20.20, TO_DATE('2002/02/02 02:22:22', 'yyyy/mm/dd hh24:mi:ss')); 
        insert into test_table03 (testvarchar, testnumber, testnumberfraction, testdate) 
            values ('szöveg03', 300, 30.30, TO_DATE('2003/03/03 03:33:33', 'yyyy/mm/dd hh24:mi:ss')); 
        insert into test_table03 (testvarchar, testnumber, testnumberfraction, testdate) 
            values ('szöveg04', 400, 40.40, TO_DATE('2004/04/04 04:44:44', 'yyyy/mm/dd hh24:mi:ss'));
        open expected_sysrefcursor for select * from test_table03;
        result_sysrefcursor := interface_pack.selectall_func(par_viewsynonymname);

        loop 
            FETCH expected_sysrefcursor INTO rec_exp_sysrefcur;
            FETCH result_sysrefcursor INTO rec_res_sysrefcur;
            /*DBMS_OUTPUT.put_line('result: '||
                                    rec_res_sysrefcur.id||' - '||
                                    rec_res_sysrefcur."TESZTSZÖVEG01"||' - '||
                                    rec_res_sysrefcur."TESZTSZÁM"||' - '||
                                    rec_res_sysrefcur."TESZTTÖRT"||' - '||
                                    rec_res_sysrefcur."TESZTDÁTUM"||' - ');
            DBMS_OUTPUT.put_line('expected: '||
                                    rec_exp_sysrefcur.id||' - '||
                                    rec_exp_sysrefcur.testvarchar||' - '||
                                    rec_exp_sysrefcur.testnumber||' - '||
                                    rec_exp_sysrefcur.testnumberfraction||' - '||
                                    rec_exp_sysrefcur.testdate||' - ');*/
            if rec_exp_sysrefcur.id <> rec_res_sysrefcur.id or rec_res_sysrefcur.id is null then
                TEST_RESULT := 'FALSE';
                exit;
            end if;
            if rec_exp_sysrefcur.testvarchar <> rec_res_sysrefcur."TESZTSZÖVEG01" or rec_res_sysrefcur."TESZTSZÖVEG01" is null then
                TEST_RESULT := 'FALSE';
                exit;
            end if;
            if rec_exp_sysrefcur.testnumber <> rec_res_sysrefcur."TESZTSZÁM" or rec_res_sysrefcur."TESZTSZÁM" is null then
                TEST_RESULT := 'FALSE';
                exit;
            end if;if rec_exp_sysrefcur.testnumberfraction <> rec_res_sysrefcur."TESZTTÖRT" or rec_res_sysrefcur."TESZTTÖRT" is null then
                TEST_RESULT := 'FALSE';
                exit;
            end if;if rec_exp_sysrefcur.testdate <> rec_res_sysrefcur."TESZTDÁTUM" or rec_res_sysrefcur."TESZTDÁTUM" is null then
                TEST_RESULT := 'FALSE';
                exit;
            end if;
            exit when expected_sysrefcursor%NOTFOUND;
        end loop;
        
        
        execute immediate 'truncate table TEST_TABLE03';
        execute immediate 'drop sequence '||constantsettergetter_pack.const_schemaowner||'.test_table03_id_seq';
        execute immediate 'CREATE SEQUENCE test_table03_id_seq START WITH 1 NOCACHE ORDER';
        execute immediate 'drop role testrole';
        dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
        
        exception 
            when others then
                TEST_RESULT := 'FALSE';
                exceptionmessage := sqlerrm;
                dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'truncate table TEST_TABLE03';
                execute immediate 'drop sequence '||constantsettergetter_pack.const_schemaowner||'.test_table03_id_seq';
                execute immediate 'CREATE SEQUENCE test_table03_id_seq START WITH 1 NOCACHE ORDER';
                execute immediate 'drop role testrole';
end;
/
-----------------------------------------------------------------------

declare
    TEST_ID varchar2(30) := '0008.005.002';
    TEST_METHOD varchar2 (60) := 'interface_pack.selectall_func';
    TEST_NAME varchar2 (120) := 'Invalid par_viewsynonymname param test with current user (PROPALLADMIN) session';
    TEST_RESULT varchar2 (5);
    par_viewsynonymname varchar2(30) := 'Teszttábla03';
    result_sysrefcursor select_pack.select_cur;
    
    expected_exception varchar2(9) := 'ORA-20002';
    exceptionmessage varchar2 (1000);

    begin
        TEST_RESULT := 'FALSE';
        execute immediate 'create role testrole';
        execute immediate 'GRANT SELECT, INSERT, UPDATE, DELETE ON PROPALLADMIN.test_table03 TO testrole';
        execute immediate 'grant testrole to PROPALLADMIN';
        result_sysrefcursor := interface_pack.selectall_func(par_viewsynonymname);
        
        dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'drop role testrole';
        exception 
            when others then
                if expected_exception = substr(sqlerrm, 1, 9) then
                    TEST_RESULT := 'TRUE';
                    exceptionmessage := sqlerrm;
                else
                    TEST_RESULT := 'FALSE';
                    exceptionmessage := sqlerrm;
                end if;
                dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'drop role testrole';
end;
/

-----------------------------------------------------------------------
-- interface_pack.selectsimplebuilder_func FUNCTION UNIT TESZT:
-----------------------------------------------------------------------
declare
    TEST_ID varchar2(30) := '0008.006.001';
    TEST_METHOD varchar2 (60) := 'select_pack.selectsimplebuilder_func';
    TEST_NAME varchar2 (120) := 'Valid param test with current user (PROPALLADMIN) session';
    TEST_RESULT varchar2 (6);
    par_viewsynonymname varchar2(30) := 'Teszttábla 03';
    par_wherecolumnaliasname varchar2 (30):= 'TESZTSZÁM';
    par_whereoperator varchar2 (10):= '=';
    par_wherecomperedvalue varchar2 (50):= '200';
    par_orderebycolumnaliasname varchar2 (30) := 'TESZTSZÖVEG01'; 
    par_orderbydesc varchar2(4) := '1';
    
    expected_sysrefcursor sys_refcursor;
    result_sysrefcursor sys_refcursor;
    expected_exception varchar2(9) := null;
    exceptionmessage varchar2 (1000);
    
    rec_exp_sysrefcur test_table03%rowtype;
    rec_res_sysrefcur "Teszttábla 03"%rowtype;
    begin
        TEST_RESULT := 'TRUE';
        execute immediate 'create role testrole';
        execute immediate 'GRANT SELECT, INSERT, UPDATE, DELETE ON PROPALLADMIN.test_table03 TO testrole';
        execute immediate 'grant testrole to PROPALLADMIN';
        insert into test_table03 (testvarchar, testnumber, testnumberfraction, testdate) 
            values ('szöveg01', 100, 10.10, TO_DATE('2001/01/01 01:22:33', 'yyyy/mm/dd hh24:mi:ss')); 
        insert into test_table03 (testvarchar, testnumber, testnumberfraction, testdate) 
            values ('szöveg02', 200, 20.20, TO_DATE('2002/02/02 02:22:22', 'yyyy/mm/dd hh24:mi:ss')); 
        insert into test_table03 (testvarchar, testnumber, testnumberfraction, testdate) 
            values ('szöveg03', 300, 30.30, TO_DATE('2003/03/03 03:33:33', 'yyyy/mm/dd hh24:mi:ss')); 
        insert into test_table03 (testvarchar, testnumber, testnumberfraction, testdate) 
            values ('szöveg04', 400, 40.40, TO_DATE('2004/04/04 04:44:44', 'yyyy/mm/dd hh24:mi:ss'));
        open expected_sysrefcursor for select * from test_table03 where testnumber = 200 order by testvarchar desc;
        result_sysrefcursor := interface_pack.selectsimplebuilder_func
        (par_viewsynonymname, par_wherecolumnaliasname, par_whereoperator, par_wherecomperedvalue, par_orderebycolumnaliasname, par_orderbydesc);
 
        loop 
            FETCH expected_sysrefcursor INTO rec_exp_sysrefcur;
            FETCH result_sysrefcursor INTO rec_res_sysrefcur;
            /*DBMS_OUTPUT.put_line('result: '||
                                    rec_res_sysrefcur.id||' - '||
                                    rec_res_sysrefcur."TESZTSZÖVEG01"||' - '||
                                    rec_res_sysrefcur."TESZTSZÁM"||' - '||
                                    rec_res_sysrefcur."TESZTTÖRT"||' - '||
                                    rec_res_sysrefcur."TESZTDÁTUM"||' - ');
            DBMS_OUTPUT.put_line('expected: '||
                                    rec_exp_sysrefcur.id||' - '||
                                    rec_exp_sysrefcur.testvarchar||' - '||
                                    rec_exp_sysrefcur.testnumber||' - '||
                                    rec_exp_sysrefcur.testnumberfraction||' - '||
                                    rec_exp_sysrefcur.testdate||' - ');*/
            if rec_exp_sysrefcur.id <> rec_res_sysrefcur.id or rec_res_sysrefcur.id is null then
                TEST_RESULT := 'FALSE';
                exit;
            end if;
            if rec_exp_sysrefcur.testvarchar <> rec_res_sysrefcur."TESZTSZÖVEG01" or rec_res_sysrefcur."TESZTSZÖVEG01" is null then
                TEST_RESULT := 'FALSE';
                exit;
            end if;
            if rec_exp_sysrefcur.testnumber <> rec_res_sysrefcur."TESZTSZÁM" or rec_res_sysrefcur."TESZTSZÁM" is null then
                TEST_RESULT := 'FALSE';
                exit;
            end if;if rec_exp_sysrefcur.testnumberfraction <> rec_res_sysrefcur."TESZTTÖRT" or rec_res_sysrefcur."TESZTTÖRT" is null then
                TEST_RESULT := 'FALSE';
                exit;
            end if;if rec_exp_sysrefcur.testdate <> rec_res_sysrefcur."TESZTDÁTUM" or rec_res_sysrefcur."TESZTDÁTUM" is null then
                TEST_RESULT := 'FALSE';
                exit;
            end if;
            exit when expected_sysrefcursor%NOTFOUND;
        end loop;
        
        
        execute immediate 'truncate table TEST_TABLE03';
        execute immediate 'drop sequence '||constantsettergetter_pack.const_schemaowner||'.test_table03_id_seq';
        execute immediate 'CREATE SEQUENCE test_table03_id_seq START WITH 1 NOCACHE ORDER';
        execute immediate 'drop role testrole';
        dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
        
        exception 
            when others then
                TEST_RESULT := 'FALSE';
                exceptionmessage := sqlerrm;
                dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'truncate table TEST_TABLE03';
                execute immediate 'drop sequence '||constantsettergetter_pack.const_schemaowner||'.test_table03_id_seq';
                execute immediate 'CREATE SEQUENCE test_table03_id_seq START WITH 1 NOCACHE ORDER';
                execute immediate 'drop role testrole';
end;
/

-----------------------------------------------------------------------

declare
    TEST_ID varchar2(30) := '0008.006.002';
    TEST_METHOD varchar2 (60) := 'interface_pack.selectsimplebuilder_func';
    TEST_NAME varchar2 (120) := 'Invalid par_viewsynonymname param test with current user (PROPALLADMIN) session';
    TEST_RESULT varchar2 (5);
    par_viewsynonymname varchar2(30) := 'Teszttábla03';
    par_wherecolumnaliasname varchar2 (30):= 'TESZTSZÁM';
    par_whereoperator varchar2 (10):= '=';
    par_wherecomperedvalue varchar2 (50):= '200';
    par_orderebycolumnaliasname varchar2 (30) := 'TESZTSZÖVEG01'; 
    par_orderbydesc varchar2(4) := '1';
    
    
    result_sysrefcursor select_pack.select_cur;
    rec_res_sysrefcur "Teszttábla 03"%rowtype;
    expected_exception varchar2(9) := 'ORA-20002';
    exceptionmessage varchar2 (1000);

    begin
        TEST_RESULT := 'FALSE';
        execute immediate 'create role testrole';
        execute immediate 'GRANT SELECT, INSERT, UPDATE, DELETE ON PROPALLADMIN.test_table03 TO testrole';
        execute immediate 'grant testrole to PROPALLADMIN';
        result_sysrefcursor := interface_pack.selectsimplebuilder_func
        (par_viewsynonymname, par_wherecolumnaliasname, par_whereoperator, par_wherecomperedvalue, par_orderebycolumnaliasname, par_orderbydesc);
        
        dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'drop role testrole';
        exception 
            when others then
                if expected_exception = substr(sqlerrm, 1, 9) then
                    TEST_RESULT := 'TRUE';
                    exceptionmessage := sqlerrm;
                else
                    TEST_RESULT := 'FALSE';
                    exceptionmessage := sqlerrm;
                end if;
                dbms_output.put_line(TEST_ID|| chr(10) ||
                            'TESTDATA:'|| chr(10) ||
                            'Method: '|| TEST_METHOD || chr(10) ||
                            'Test name: ' || TEST_NAME || chr (10) ||
                            'Exception message: ' || exceptionmessage || chr (10) ||
                            'Result: '|| TEST_RESULT); 
                execute immediate 'drop role testrole';
end;
/